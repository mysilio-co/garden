...
export default function NotePage({
  ...
  const {
    ...
    save: saveItem,
    resource: garden,
    saveResource: saveGarden,
  } = useTitledGardenItem(gardenUrl, itemName)
  ...
  async function maybeSaveNoteBody(newValue) {
    if (newValue && !equal(newValue, value)) {
      ...
      let newNoteResource = noteBodyThings.reduce(
    ...
    >
      ...
      {bookmarkUrl && (
        <div className={`px-4 flex flex-row ${bg} text-white`}>
          ...
          <a target="_blank" rel="noreferrer noopener" href={bookmarkUrl}>
            {bookmarkUrl}
          </a>
        </div>
      )}
      {fileUrl && (
        <div className={`px-4 pb-4 flex flex-row ${bg} text-white`}>
          ...
          <a target="_blank" rel="noreferrer noopener" href={fileUrl}>
            {fileUrl}
          </a>
        </div>
      )}
      <div className="relative">
        ...
        >
          <div className="absolute top-0 right-0 text-gray-500 bg-gray-100 rounded-bl-lg z-40 flex flex-col">
            <button
              onClick={() => setCoverImageUploaderOpen(true)}
            ...
            >
              ...
              <ImageUploadModal
                open={coverImageUploaderOpen}
                setOpen={setCoverImageUploaderOpen}
                onSave={(url) => setCoverImage(url)}
                uploadContainerUri={imageUploadUri}
              />
            </button>
            <button
