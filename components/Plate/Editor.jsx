...
function ConceptStartLeaf({ children }) {
  return <span className="opacity-50 group-hover:opacity-100">{children}</span>
}
...
const CodeBlockElement = ({ attributes, children, element, nodeProps }) => {
  return (
    <pre
      {...attributes}
      {...nodeProps}
      className="bg-gray-100 px-4 py-2 ml-4 mt-2"
    >
      <code>{children}</code>
    </pre>
  )
...
function LinkElement({ attributes, children, element, nodeProps }) {
  return (
    <>
      <a
        className="text-my-purple underline"
        href={element.url}
        {...attributes}
        {...nodeProps}
      >
...
function CodeMark({ className = '', attributes, children, nodeProps }) {
  return (
    <span
      className={`font-mono bg-gray-200 ${className}`}
      {...attributes}
      {...nodeProps}
    >
      {children}just
...
function HighlightMark({ className = '', attributes, children, nodeProps }) {
  return (
    <span
      className={`bg-yellow-200 ${className}`}
      {...attributes}
      {...nodeProps}
    >
      {children}
...
function onConceptSelect(editor, item) {
  if (item) {
    Transforms.insertText(editor, `[[${item.text}]]`, {
      at: comboboxStore.get.targetRange(),
    })
  }
}
...
export default function Editor({
  ...
  return (
    ...
    >
      ...
      <Plate id={editorId} editableProps={editableProps} {...props}>
        {
          // TODO: set permissions on uploaded images the same way we do with cover images, then bring this back
          false && !readOnly && (
            <>
              ...
              <Combobox
                ...
                items={tagItems}
                trigger="#"
                onSelectItem={onTagSelect}
              />
              <Combobox
                id="conceptCombobox"
                items={conceptItems}
                trigger="[["
                onSelectItem={onConceptSelect}
              />
            </>
          )
        }
